...
import { cancelBooking } from "@/lib/fetches/profile";
...
export default function UserBookings() {
  ...
  const handleConfirmCancel = async () => {
    ...
    setCancelingId(selectedBookingToCancel.id);
    ...
    try {
      ...
      setBookingsData((prevData) =>
        prevData.map((booking) =>
          booking.id === selectedBookingToCancel.id
            ? { ...booking, booking_status: "3" } // 3 = Cancelled
            : booking,
        ),
      );

      // Optional: Show success toast/notification (you can add this)
  ...
  const canCancelBooking = (booking: Booking): boolean => {
    // Can only cancel if status is Open (1) or Accepted (2)
  ...
  return (
    <div className="space-y-6 px-4 sm:px-6 md:px-12 py-8">
                ...
                <td className="px-6 py-4 text-center">
                  {canCancelBooking(b) ? (
                    <button
                      ...
                      className="px-3 py-1 rounded text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:bg-gray-400 transition-colors"
                    >
            ...
            <div className="border-t pt-4 space-y-3">
              ...
              {canCancelBooking(b) && (
                <button
                  onClick={() => handleCancelClick(b)}
                  disabled={cancelingId === b.id}
                  className="w-full mt-3 px-4 py-2 rounded-lg text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:bg-gray-400 transition-colors"
                >
      ...
      {/* Confirmation Modal */}
      {showConfirmModal && selectedBookingToCancel && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg max-w-sm w-full p-6 animate-in fade-in">
            ...
            <p className="text-gray-600 mb-6">
              ...
              <span className="font-semibold">
                {selectedBookingToCancel.order_id}
              </span>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={cancelingId === selectedBookingToCancel.id}
                className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 disabled:bg-gray-400 transition-colors"
              >
                {cancelingId === selectedBookingToCancel.id
                  ? "Cancelling..."
                  : "Yes, Cancel"}
              </button>
            </div>
