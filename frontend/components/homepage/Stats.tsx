  ...
  return (
    <div className="grid gap-2 grid-cols-3 sm:gap-3">
      {items.map(({ icon: Icon, title, lines }) => (
        <div
          key={title}
          className="flex  sm:flex-row items-center sm:items-start justify-center sm:justify-start text-center sm:text-left gap-1 sm:gap-2 rounded-lg sm:rounded-lg border border-brand-raiden-100 bg-white p-2 sm:p-3 shadow-sm"
        >
          ...
          <div className="min-w-0 flex-1 font-general-sans text-brand-raiden-600">
            <div className="text-[9px] sm:text-[13px] font-semibold leading-tight">
              {title}
            </div>

            <div className="mt-0.5 hidden sm:block space-y-0.5 text-[10px] font-medium leading-snug text-gray-600">
              {lines.map((line) => (
